 ...
 # asociacion
  ...
  output$data <- DT::renderDataTable({
    if(input$selec == "Características de población y migración"){
      ...
      DT::datatable(read.table(file='./CSV/ps/Isla-Mujeres-230030001-2.csv', sep=',', header = TRUE, stringsAsFactors = FALSE, fileEncoding="latin1")[, input$show_vards, drop = FALSE])
    }
  } )
  ...
  output$sum <- renderPrint({
    if(input$selec == "Características de población y migración"){
      ...
      summary(read.table(file='./CSV/ps/Isla-Mujeres-230030001-2.csv', sep=',', header = TRUE, stringsAsFactors = FALSE, fileEncoding="latin1")[, input$show_vards, drop = FALSE])
    }
  })
  ...
  lect2 <- function(){
    df_ide <- read.table(file='./CSV/ps/Isla-Mujeres-230030001-2.csv', sep=',', header = TRUE, stringsAsFactors = FALSE, fileEncoding="latin1")
  ...
  datasetComplete <- read.table(file='./CSV/ps/columnas1.csv', sep=',', header = TRUE, stringsAsFactors = FALSE, fileEncoding="latin1")[c(5:6,15:21,23:28,30:34,36, 37:51, 53:58,62,79,84:94,96,107:113,115,120,126,133:138,140,146:148,152,153,157,159,161,162,164:167,169:171)]
  datasetCompletes <- read.table(file='./CSV/ps/Isla-Mujeres-230030001-2.csv', sep=',', header = TRUE, stringsAsFactors = FALSE, fileEncoding="latin1")[,c(6,8:14,18,22:28,31:34,36,37,39,41:49,62:66,69:71,73,74,76:81,95:98,101,103:121,123:128,130:132,135:140,142:144,146:150,152,153,157:160,162:164,168:172,174:185,187:190,193:197,199:210)]
  ...
  for(i in 1:ncol(datasetComplete)){
    local({
      ii <- i
      title <- c("Sexo del encuestado","Edad del encuestado", # "Direccion.Colonia"="Colonia", 
                 "Número de adultos que viven en casa","Número de niños que viven en casa","¿Cuántos integrantes de la familia van al kinder?", "¿Cuántos integrantes de la familia van en primaria?","¿Cuántos integrantes de la familia van en secundaria?","¿Cuántos integrantes de la familia van en bachillerato?","¿Cuántos integrantes de la familia van en licenciatura?"
                 ,"¿Utilizan autobus?","¿Utilizan colectivo?","¿Utilizan taxi?","¿Utilizan mototaxi?","¿Utilizan moto?","¿Utilizan auto?",
                 "Número de personas que compran en el mercado de la colonia","Número de personas que compran en abarrotes","Número de personas que compran en super","Número de personas que compran en tienda de conveniencia","Número de personas que compran en plaza comercial",
                 "En caso de emergencia se atienden en casa",
                 "En caso de emergencia se atienden en medico particular de la comunidad", "En caso de emergencia se atienden en medico particular de Cancun","En caso de emergencia se atienden en Hospital",
                 "En caso de emergencia se atienden en cruz roja","En caso de emergencia se atienden en farmacia","En caso de emergencia se atienden en otro","En su colonia cuentan con parque","En su colonia cuentan con unidad deportiva",
                 "En su colonia cuentan con jardin","En su colonia cuentan con centro comunitario","En su colonia cuentan con biblioteca","En su colonia cuentan con otro tipo de instalación","¿Cuántas personas ademas del jefe de la familia trabajan en el hogar con ingresos pagados?",
                 "¿Cuál es el trabajo del jefe de la familia?","¿Cuál es el puesto del jefe?","¿ A cuánto asciende el ingreso semanal del jefe de la familia?","Máximo nivel completo de estudios del jefe o jefa  de familia Opciones","¿Número de personas que no perciben ingreso económico?",
                 "¿Cuánto tiempo tarda el jefe o jefa de familia en llegar a su lugar de trabajo?","¿En qué colonia trabaja el jefe o jefa de familia?","¿En qué municipio trabaja el jefe o jefa de familia?","Estado de origen del encuestado",
                 "¿Cuánto tiempo lleva viviendo en este lugar?","El motivó de vivir en esta la localidad son parientes","El motivó de vivir en esta la localidad son amigos","El motivó de vivir en esta la localidad es trabajo","El motivó de vivir en esta la localidad es poner un negocio",
                 "El motivó de vivir en esta la localidad es oportunidad de vivir mejor","El motivó de vivir en esta la localidad es otro","En caso de requerir ayuda, apoyo legal, económico o familiar ante algún, problema acudo con mis vecinos","En caso de requerir ayuda, apoyo legal, económico o familiar ante algún, problema acudo con mi familia","En caso de requerir ayuda, apoyo legal, económico o familiar ante algún, problema acudo a la autoridad","En caso de requerir ayuda, apoyo legal, económico o familiar ante algún, problema acudo a la iglesia",
                 "En caso de requerir ayuda, apoyo legal, económico o familiar ante algún, problema acudo con otro","¿Qué religión practica?",
                 "¿Cuáles son las ventajas de vivir en esta comunidad? Tengo casa","¿Cuáles son las ventajas de vivir en esta comunidad? Tengo trabajo","¿Cuáles son las ventajas de vivir en esta comunidad? Tengo familia","¿Cuáles son las ventajas de vivir en esta comunidad? Tengo más tiempo",
                 "¿Cuáles son las ventajas de vivir en esta comunidad? Es tranquilo","¿Cuáles son las ventajas de vivir en esta comunidad? Es seguro","¿Cuáles son las ventajas de vivir en esta comunidad? Otro","¿Piensa irse a vivir a otra localidad?","¿Usted a qué municipio siente que pertenece?","¿Qué tan frecuente va a la Isla, la cabecera municipal de Isla Mujeres?","¿Cuales son los motivos por los que viaja a isla?  Asuntos administrativos",
                 "¿Cuales son los motivos por los que viaja a isla? Pagos de servicios","¿Cuales son los motivos por los que viaja a isla? Trabajo","¿Cuales son los motivos por los que viaja a isla? Recreación","¿Cuales son los motivos por los que viaja a isla? Visita de la familia","¿Cuales son los motivos por los que viaja a isla? Otro",
                 "La vivienda que habita tu familia es:","¿De que forma fue adquirida? De contado","¿De que forma fue adquirida? Herencia","¿De que forma fue adquirida? Mensual",
                 "¿Cuántas inundaciones ha sufrido al vivir en esta localidad?",
                 "¿Le ha tocado enfrentar el golpe de un huracán, viviendo en esta localidad?","¿Sabe si su vivienda está en zona de riesgo?","¿Conoce los impactos o afectaciones que puede sufrir?","Alrededor de la vivienda se cuenta con áreas verdes","Alrededor de la vivienda se cuenta con banquetas","Alrededor de la vivienda se cuenta con luminarias","Alrededor de la vivienda se cuenta con transporte público",
                 "Alrededor de la vivienda se cuenta con patrullas vigilando","Alrededor de la vivienda se cuenta con lotes baldíos","¿Qué percepción tuvo el encuestador de la vivienda y la calle? Percepción seguridad","¿Qué percepción tuvo el encuestador de la vivienda y la calle? Percepción comodidad","¿Qué percepción tuvo el encuestador de la vivienda y la calle? Percepción de riesgo"
        ...
        if(is.numeric(datasetComplete[[ii]])){
          if( length(unique(datasetComplete[[ii]]))==2 ){
            ggplot( data=datasetComplete, aes_string(names(datasetComplete)[ii]) ) +
              ...
              scale_x_continuous(name = title, breaks = c(0, 1))+
  ...
  output$tableInfos <- renderDT({
    sketch <- tags$table(
      class = "row-border stripe hover compact",
      tableHeader(c("", c("Participa.Eventos.Deportivos",	"Participa.En.Fiestas",	"Participa.En.Iglesia.Templo",	"Participa.Solucion.Problemas.Comunidad"="Part.Solución.Problemas.Comunidad",
                          "Conoce.Vecinos",	"Conoce.Vecinos.Confiaria.Ninos"="Conoce.Vecinos.Confiaria.Niños",	"Conoce.Vecinos.Confiaria.Casa", "Conoce.Vecinos.Part.Mejorar.Seguridad",	"Part.Con.Autoridad.Mejorar.Seguridad",
                          "Cuando.Delito.Vecinos.Se.Reunen"="Cuando.Delito.Vecinos.Se.Reúnen",	"Cuando.Delito.Organizan.Para.Vigilar","Cuando.Delito.Vecinos.Intercambian.Num.Tel"="Cuando.Delito.Intercambian.Núm.Tel",	"Cuando.Delito.Vecinos.Forman.Chat",	"Cuando.Delito.Ponen.Advertencia",
                          "Cuando.Delito.Vecinos.Llaman.Policia"="Cuando.Delito.Vecinos.Llaman.Policía",	"Cuando.Delito.Denuncian.A.Autoridad",	"Durante.Ultimo.Ano.Hubo.Robo.casa"="Durante.Último.Año.Hubo.Robo.Casa",	"Durante.Ultimo.Ano.Hubo.Robo.Calle"="Durante.Último.Año.Hubo.Robo.Calle","Durante.Ultimo.Ano.Hubo.Robo.Transporte"=	"Durante.Últ.Año.Hubo.Robo.Transporte",
                          "Durante.Ultimo.Ano.Hubo.Robo.Negocio"="Durante.Últ.Año.Robo.Negocio",	"Durante.Ultimo.Ano.Hubo.Robo.De.Vehiculo"="Durante.Últ.Año.Robo.De.Vehiculo","Durante.Ultimo.Ano.Hubo.Balaceras"="Durante.Último.Año.Hubo.Balaceras",	"Durante.Ultimo.Ano.Hubo.Violencia.Familiar"="Durante.Últ.Año.Violencia.Familiar",	"Riesgo.Sufrir.Delito.En.Casa",	"Riesgo.Sufrir.Delito.En.Calle",
                          "Riesgo.Sufrir.Delito.En.Esta.Zona",	"Riesgo.Sufrir.Delito.En.Esta.Ciudad",	"Ha.Sido.Victima.Delito.Ultimo.Ano"="Ha.Sido.Victima.Delito.Últ.Año",	"Caso.Ser.Victima.Delito.Llama.Policia"="Ser.Victima.Delito.Llama.Policía",	"Caso.Ser.Victima.Delito.Hace.Denuncia"="Ser.Victima.Delito.Hace.Denuncia",
                          "Caso.Ser.Victima.Delito.Advierte.Vecinos"="Ser.Victima.Delito.Advierte.Vecinos",	"Caso.Ser.Victima.Delito.Advierte.Familia"="Ser.Victima.Delito.Advierte.Familia",	"Padres.Participan.Actividades.Con.Hijos"="Padres.Part.Actividades.Con.Hijos",	"Vecinos.Organizan.Prevenir.Delitos",	"Hay.Personas.Amables",
                          "Hay.Personas.Que.Siempre.Ayudan.A.Otros"="Hay.Personas.Que.Ayudan.Otros",	"Hay.Personas.A.Las.Que.Todos.Tienen.Miedo"="Hay.Personas.A.Las.Que.Todos.Temen",	"Hay.Personas.Que.Se.Emborrachan.O.Drogan"="Hay.Personas.Emborrachan.O.Drogan","Hay.Personas.Que.Han.Estado.Carcel"=	"Hay.Personas.Han.Estado.Cárcel",	"Hay.Personas.Sospechosas",
                          "Hay.Violencia.Entre.Hombres",	"Hay.Violencia.Entre.Familias",	"Hay.Violencia.Entre.Jovenes"="Hay.Violencia.Entre.Jóvenes",	"Si.Conflictos.Concilian.Con.3ra.Persona",	"si.Conflictos.Entre.Vecinos.Dialogando"="Conflictos.Entre.Vecinos.Dialogando",	"si.Conflictos.Concilian.Respetuosamente"="Conflictos.Concilian.Respetuosamente",
                          "si.Conflictos.Se.Manejan.A.Gritos"="Si.Conflictos.Se.Manejan.A.Gritos",	"si.Conflictos.Tratan.Con.Golpes"="Si.Conflictos.Tratan.Con.Golpes",	"Jovenes.Hacen.Deporte"="Jóvenes.Hacen.Deporte",	"Jovenes.Ayudan.Otros"="Jóvenes.Ayudan.Otros",	"Mayoria.De.Jovenes.Estudian.Trabajan"="Mayoría.De.Jóvenes.Estudian.Trabajan",
                          "Jovenes.Andan.Pandillas"="Jóvenes.Andan.Pandillas",	"Jovenes.Son.Violentos"="Jóvenes.Son.Violentos",	"Hay.Parque",	"Hay.Parque.En.Buen.Estado",	"Hay.Parque.Utilizado.Por.Ninos"="Hay.Parque.Utilizado.Por.Niños",	"Hay.Parque.Utilizado.Por.Jovenes"="Hay.Parque.Utilizado.Por.Jóvenes",	"Hay.Parque.Utilizado.Por.Pandillas",	"Hay.Parque.Utilizado.Por.Familias",
                          "Parque.Utilizado.Por.Tercera.Edad",	"Hay.Parque.Utilizado.Por.Adultos",	"Actividades.Supervisadas.Por.Adultos",	"Hay.Parque.Utilizado.Por.Vandalos"="Hay.Parque.Utilizado.Por.Vándalos",	"Parque.Utilizado.Por.Otras.Zonas",
                          "Hay.Parque.Utilizado.Por.Usted",	"Hay.Banquetas",	"Hay.Baches",	"Hay.Letreros.De.Calles",	"Hay.Tiendita",	"Hay.Alumbrado",	"Hay.Consumo.Alcohol.En.Calle",	"Hay.Horarios.Transporte.Convenientes",	"Hay.Terrenos.Baldios"="Hay.Terrenos.Baldíos",
                          "Hay.Basura",	"Hay.Autos.Abandonados",	"Hay.Casas.Abandonadas",	"Hay.Vandalismo",	"Hay.Grafiti",	"Hay.Venta.Alcohol.Cigarros.A.Menores",	"Hay.Venta.Droga",	"Hay.Venta.Alcohol.Despues.De.Once.Noche"="Venta.Alcohol.Después.Once.Noche",	"Algun.Menor.Abandono.Escuela"="Algún.Menor.Abandono.Escuela",	"Algun.Menor.Tiene.Problemas.Conducta"="Menor.Tiene.Problemas.Conducta",	"Algun.Menor.Quedo.Embarazada"="Algún.Menor.Quedo.Embarazada",	"Corregir.Ninos.Recomienda.Castigarle"="Corregir.Niños.Recomienda.Castigarle",	"Corregir.Ninos.Recomienda.Gritarle"="Corregir.Niños.Recomienda.Gritarle",
                          "Corregir.Ninos.Recomienda.Darle.Nalgadas"="Corregir.Niños.Recomienda.Nalgadas",	"Corregir.Ninos.Recomienda.Explicarle.Esta.Mal"="Corregir.Recomienda.Explicar.Esta.Mal",	"Corregir.Ninos.Recomienda.Aconsejarle"="Corregir.Niños.Recomienda.Aconsejar",	"Corregir.Ninos.Recomienda.Ensenar.Ejemplo"="Corregir.Recomienda.Enseñar.Ejemplo",	"En.Casa.Platican.Unos.Con.Otros",	"En.Casa.Comen.Juntos",	"En.Casa.Se.Ayudan.Gastos",	"En.Casa.Discuten",	"En.Casa.Se.Gritan",	"En.Casa.Se.Ignoran",	"En.Casa.Alguien.Tiene.Discapacidad",	"En.Casa.Alguien.No.Habla.Espanol"="En.Casa.Alguien.No.Habla.Español",	"En.Casa.Alguien.Necesita.Ayuda.Obesidad"="Alguien.Necesita.Ayuda.Obesidad",	"En.Casa.Alguien.Necesita.Ayuda.Fumar"="Alguien.Necesita.Ayuda.Fumar",	"En.Casa.Alguien.Necesita.Ayuda.Beber"="Alguien.Necesita.Ayuda.Beber",
                          "Ultimo.Ano.Por.Seguridad.Ha.Pensado.Cambiarse.Casa"="Últ.Año.Ha.Pensado.Cambiarse.Casa","Ultimo.Ano.Por.Seguridad.Ha.Pensado.Cambiarse.Ciudad"="Últ.Año.Ha.Pensado.Cambiarse.Ciudad",	"Ultimo.Ano.Por.Seguridad.Ha.Pensado.Cambiarse.Estado"="Últ.Año.Ha.Pensado.Cambiarse.Estado",	"Ultimo.Ano.Por.Seguridad.Ha.Pensado.Dejo.Salir.Noche"="Últ.Año.Ha.Pensado.Dejó.Salir.Noche",	"Ultimo.Ano.Por.Seguridad.Ha.Pensado.Dejo.Salir.Noche"="Últ.Año.Ha.Pensado.Dejó.Salir.Noche",	"Ultimo.Ano.Por.Seguridad.Impidio.Ninos.Salieran.Calle"="Últ.Año.Impidio.Niños.Salieran.Calle",	"Ult.Ano.Por.Seguridad.Evito.Relacionarse.Nuevas.Personas"="Últ.Año.Evito.Nuevas.Personas",	"Ultimo.Ano.Por.Seguridad.Dejo.De.Visitar.Parientes.Amigos"="Últ.Año.no.visito.Parientes.Amigos",	"Ultimo.Ano.Por.Seguridad.Dejo.Usar.Taxi"="Últ.Año.Dejó.Usar.Taxi",	"Ultimo.Ano.Por.Seguridad.Dejo.De.Llevar.Mucho.Efectivo"="Últ.Año.Dejó.De.Llevar.Mucho.Efectivo",	"Ultimo.Ano.Por.Seguridad.Dejo.De.Usar.Joyas"="Últ.Año.Dejó.De.Usar.Joyas",	
                          "Policia.Cuida.Vigila.Bien"="Policía.Cuida.Vigila.Bien",	"Policia.Comete.Abusos"="Policía.Comete.Abusos",	"Policia.Acude.Llamados"="Policía.Acude.Llamados",	"Policia.Pide.Mordidas"="Policía.Pide.Mordidas",	"Policia.Hace.Rondines"="Policía.Hace.Rondines",	"Policia.Comete.Delitos"="Policía.Comete.Delitos",	"Confianza.Que.Tiene.En.La.Policia"="Confianza.Que.Tiene.En.La.Policía",	"Confianza.Que.Tiene.En.Ministerio.Publico"="Confianza.En.Ministerio.Público",	"Confianza.Que.Tiene.En.La.Institucion.Educativa.De.Zona"="Confianza.En.Instit.Educativa.Zona",	
                          "Confianza.Que.Tiene.En.Su.Presidente.Municipal"="Confianza.En.Presidente.Municipal",	"Confianza.Que.Tiene.En.El.Gobernador",	"Calificacion.Trabajo.De.La.Policia"="Calif.Trabajo.De.Policía",	"Calificacion.Trabajo.Del.Ministerio.Publico.Para.Denunciar"="Calif.Trabajo.Ministerio.al.Denunciar",	"Calificacion.Trabajo.Presidente.Municipal"="Calif.Trabajo.Presidente.Municipal",	"Calificacion.Trabajo.Del.Gobernador"="Calif.Trabajo.Del.Gobernador",	"Calificacion.Trato.Que.Da.La.Policia"="Calif.Trato.Que.Da.Policía", "Calificacion.Trato.Que.Da.Ministerio.Publico.Para.Denunciar"="Calif.Trato.De.Ministerio.al.Denunciar",	
                          "Calificacion.Trato.Recibe.De.Los.Empleados.De.Gobierno"="Calif.Trato.De.Empleados.De.Gobierno",	"Calificacion.Trato.Recibe.De.Su.Presidente.Municipal"="Calif.Trato.De.Su.Presidente.Municipal",	"Calificacion.Trato.Recibe.Del.Gobernador"="Calif.Trato.Recibe.Del.Gobernador",	"Sexo",	"Num.Personas.Viven.Esta.Casa"="Núm.Personas.Viven.Esta.Casa",	"Cuantos.Hombres"="Cuántos.Hombres",	"Cuantas.Mujeres"="Cuántas.Mujeres",	"Cuantos.Menores"="Cuántos.Menores",	"La.Madre.De.Los.Menores.Vive.En.Casa",	"El.Padre.De.Los.Menores.Vive.En.Casa",	"Esta.Casa.Es.Propia",	"Tiempo.Viviendo.En.Q.Roo",	"Tiempo.Viviendo.En.Esta.Casa"
      )
  ...
  output$modalss <- renderUI({
    lapply(1:ncol(datasetCompletes), function(i){
      bsModal(
        id = paste0("modals",i),
        title = c("Participa.Eventos.Deportivos",	"Participa.En.Fiestas",	"Participa.En.Iglesia.Templo",	"Participa.Solucion.Problemas.Comunidad"="Part.Solución.Problemas.Comunidad",
                  "Conoce.Vecinos",	"Conoce.Vecinos.Confiaria.Ninos"="Conoce.Vecinos.Confiaria.Niños",	"Conoce.Vecinos.Confiaria.Casa", "Conoce.Vecinos.Part.Mejorar.Seguridad",	"Part.Con.Autoridad.Mejorar.Seguridad",
                  "Cuando.Delito.Vecinos.Se.Reunen"="Cuando.Delito.Vecinos.Se.Reúnen",	"Cuando.Delito.Organizan.Para.Vigilar","Cuando.Delito.Vecinos.Intercambian.Num.Tel"="Cuando.Delito.Intercambian.Núm.Tel",	"Cuando.Delito.Vecinos.Forman.Chat",	"Cuando.Delito.Ponen.Advertencia",
                  "Cuando.Delito.Vecinos.Llaman.Policia"="Cuando.Delito.Vecinos.Llaman.Policía",	"Cuando.Delito.Denuncian.A.Autoridad",	"Durante.Ultimo.Ano.Hubo.Robo.casa"="Durante.Último.Año.Hubo.Robo.Casa",	"Durante.Ultimo.Ano.Hubo.Robo.Calle"="Durante.Último.Año.Hubo.Robo.Calle","Durante.Ultimo.Ano.Hubo.Robo.Transporte"=	"Durante.Últ.Año.Hubo.Robo.Transporte",
                  "Durante.Ultimo.Ano.Hubo.Robo.Negocio"="Durante.Últ.Año.Robo.Negocio",	"Durante.Ultimo.Ano.Hubo.Robo.De.Vehiculo"="Durante.Últ.Año.Robo.De.Vehículo","Durante.Ultimo.Ano.Hubo.Balaceras"="Durante.Último.Año.Hubo.Balaceras",	"Durante.Ultimo.Ano.Hubo.Violencia.Familiar"="Durante.Últ.Año.Violencia.Familiar",	"Riesgo.Sufrir.Delito.En.Casa",	"Riesgo.Sufrir.Delito.En.Calle",
                  "Riesgo.Sufrir.Delito.En.Esta.Zona",	"Riesgo.Sufrir.Delito.En.Esta.Ciudad",	"Ha.Sido.Victima.Delito.Ultimo.Ano"="Ha.Sido.Victima.Delito.Últ.Año",	"Caso.Ser.Victima.Delito.Llama.Policia"="Ser.Victima.Delito.Llama.Policía",	"Caso.Ser.Victima.Delito.Hace.Denuncia"="Ser.Victima.Delito.Hace.Denuncia",
                  "Caso.Ser.Victima.Delito.Advierte.Vecinos"="Ser.Victima.Delito.Advierte.Vecinos",	"Caso.Ser.Victima.Delito.Advierte.Familia"="Ser.Victima.Delito.Advierte.Familia",	"Padres.Participan.Actividades.Con.Hijos"="Padres.Part.Actividades.Con.Hijos",	"Vecinos.Organizan.Prevenir.Delitos",	"Hay.Personas.Amables",
                  "Hay.Personas.Que.Siempre.Ayudan.A.Otros"="Hay.Personas.Que.Ayudan.Otros",	"Hay.Personas.A.Las.Que.Todos.Tienen.Miedo"="Hay.Personas.A.Las.Que.Todos.Temen",	"Hay.Personas.Que.Se.Emborrachan.O.Drogan"="Hay.Personas.Emborrachan.O.Drogan","Hay.Personas.Que.Han.Estado.Carcel"=	"Hay.Personas.Han.Estado.Cárcel",	"Hay.Personas.Sospechosas",
                  "Hay.Violencia.Entre.Hombres",	"Hay.Violencia.Entre.Familias",	"Hay.Violencia.Entre.Jovenes"="Hay.Violencia.Entre.Jóvenes",	"Si.Conflictos.Concilian.Con.3ra.Persona",	"si.Conflictos.Entre.Vecinos.Dialogando"="Conflictos.Entre.Vecinos.Dialogando",	"si.Conflictos.Concilian.Respetuosamente"="Conflictos.Concilian.Respetuosamente",
  ...
  for(i in 1:ncol(datasetCompletes)){
    local({
      ii <- i
      title = c("En esta calle o zona, ¿usted participa en eventos deportivos?","En esta calle o zona, ¿usted participa en fiestas?","En esta calle o zona, ¿usted participa en iglesia o templo?", "En esta calle o zona, ¿usted participa en la solición de problemas de la comunidad?"
                ,"En esta calle o zona, ¿usted conoce a sus vecinos?","¿Les confiaría sus niños a los vecinos?", "¿Les confiaría su casa a los vecinos?", "¿Participa con los vecinos para mejorar la seguridad?", "¿Participa con la autoridad para mejorar la seguridad?",
                "Cuando hay un delito, en esta calle o zona, ¿los vecinos se reúnen?", "Cuando hay un delito, en esta calle o zona, ¿los vecinos se organizan para vigilar?","Cuando hay un delito, en esta calle o zona, ¿los vecinos intercambian números telefónicos?","Cuando hay un delito, en esta calle o zona, ¿los vecinos forman un chat?","Cuando hay un delito, en esta calle o zona, ¿los vecinos ponen letreros de advertencia?", "Cuando hay un delito, en esta calle o zona, ¿los vecinos llaman a la policía?", "Cuando hay un delito, en esta calle o zona, ¿los vecinos denuncian ante la autoridad?",
                "¿Durante el último año, en esta calle o zona ha habido robo en casa?", "¿Durante el último año, en esta calle o zona ha habido robo en la calle?", "¿Durante el último año, en esta calle o zona ha habido robo en el tranporte público?", "¿Durante el último año, en esta calle o zona ha habido robo a negocio?","¿Durante el último año, en esta calle o zona ha habido robo de vehículo?","¿Durante el último año, en esta calle o zona ha habido balaceras?","¿Durante el último año, en esta calle o zona a habido violencia familiar?",
                "Calificación del 1 al 5 siendo 1 el más bajo y 5 el más alto del riesgo de sufrir un delito en casa","Calificación del 1 al 5 siendo 1 el más bajo y 5 el más alto del riesgo de sufrir un delito en esta calle","Calificación del 1 al 5 siendo 1 el más bajo y 5 el más alto del riesgo de sufrir un delito en esta zona", "Calificación del 1 al 5 siendo 1 el más bajo y 5 el más alto del riesgo de sufrir un delito en esta ciudad",
                "¿Usted ha sido víctima de algún delito en el último año?", "En caso de ser víctima del delito ¿usted llama a la policía?", "En caso de ser víctima del delito ¿usted hace una denuncia?","En caso de ser víctima del delito ¿usted advierte a sus vecinos del peligro?","En caso de ser víctima del delito ¿usted advierte a su familia del peligro?",
                "En esta calle o zona, los padres participan en actividades con hijos", "En esta calle o zona, los vecinos se organizan para prevenir delitos","En esta calle o zona, las personas son amables","En esta calle o zona, hay alguna persona que siempre ayuda a los demás", 
                "En esta calle o zona hay personas a las que todos tienen miedo","En esta calle o zona hay personas que se emborrachan o se drogan","En esta calle o zona hay personas que han estado en la cárcel","En esta calle o zona hay personas sospechosas",
                "En esta calle o zona hay violencia entre hombres", "En esta calle o zona hay violencia entre familias","En esta calle o zona hay violencia entre jóvenes", "En esta calle o zona, cuando hay conflictos entre vecinos se manejan amistosamente, con tercera persona","En esta calle o zona, cuando hay conflictos entre vecinos se manejan dialogando","En esta calle o zona, cuando hay conflictos entre vecinos se manejan de manera respetuosa", "En esta calle o zona, cuando hay conflictos entre vecinos se manejan a gritos", "En esta calle o zona, cuando hay conflictos entre vecinos se manejan con golpes" ,
                "En esta calle o zona hay jóvenes que hacen deporte", "En esta calle o zona hay jóvenes que ayudan a los demás", "En esta calle o zona la mayoría de los jóvenes estudian o trabajan","En esta calle o zona hay jóvenes que andan en pandillas","En esta calle o zona hay jóvenes que son violentos", "En esta calle o zona hay un parque",
                "En esta calle o zona hay un parque que está en buen estado","En esta calle o zona hay un parque utilizado por niños","En esta calle o zona hay un parque utilizado por jóvenes","En esta calle o zona hay un parque utilizado por pandillas","En esta calle o zona hay un parque utilizado por familias","En esta calle o zona hay un parque utilizado por personas de la tercera edad","En esta calle o zona hay un parque utilizado por adultos","En esta calle o zona hay un parque con actividades supervisadas por adultos","En esta calle o zona hay un parque utilizado por vandalos","En esta calle o zona hay un parque utilizado por personas de otras zonas","En esta calle o zona hay un parque utilizado por usted",
                "En esta calle o zona hay banquetas", "En esta calle o zona hay baches", "En esta calle o zona hay letreros con nombres de las calles","En esta calle o zona hay tiendita","En esta calle o zona hay alumbrado","En esta calle o zona hay consumo de alcohol en la calle","En esta zona hay horarios de transporte que convienen","En esta zona hay terrenos baldíos", "En esta zona hay basura","En esta zona hay autos abandonados", "En esta zona hay casas abandonadas", "En esta zona hay vandalismo","En esta zona hay grafiti","En esta zona hay venta de alcohol o cigarros a menores","En esta zona hay venta de droga","En esta zona hay venta de alcohol después de las once de la noche",
                "En el último año usted supo que algún menor de 18 años abandonó la escuela","En el último año usted supo que algún menor de 18 años tiene problemas de conducta","En el último año usted supo que algún menor de 18 años que quedó embarazada", "Para corregir a un niño o niña que se porta mal, usted recomienda castigarle","Para corregir a un niño o niña que se porta mal, usted recomienda gritarle","Para corregir a un niño o niña que se porta mal, usted recomienda darle nalgadas","Para corregir a un niño o niña que se porta mal, usted recomienda explicarle lo que está mal","Para corregir a un niño o niña que se porta mal, usted recomienda aconsejarle","Para corregir a un niño o niña que se porta mal, usted recomienda darle buen ejemplo"#,
                # "En esta casa todos se conocen",
                ,"En esta casa platican unos con otros","En esta casa comen juntos","En esta casa se ayudan con los gastos","En esta casa discuten","En esta casa se gritan entre sí","En esta casa se ignoran",
                "En esta casa alguien tiene discapacidad","En esta casa alguien no habla español","En esta casa alguien necesita ayuda por obesidad","En esta casa alguien necesita ayuda por fumar","En esta casa alguien necesita ayuda por beber",
                "En el último año, por cuestiones de seguridad usted ha pensado cambiarse de casa","En el último año, por cuestiones de seguridad usted ha pensado cambiarse de ciudad","En el último año, por cuestiones de seguridad usted ha pensado cambiarse de estado",
                "En el último año, por cuestiones de seguridad usted dejó de salir de noche","En el último año, por cuestiones de seguridad usted dejó de salir a caminar o hacer ejercicio","En el último año, por cuestiones de seguridad usted impidió que los niños salgan a la calle","En el último año, por cuestiones de seguridad usted dejó de  relacionarse con nuevas personas","En el último año, por cuestiones de seguridad usted dejó de  visitar a parientes o amigos","En el último año, por cuestiones de seguridad usted dejó de usar taxi","En el último año, por cuestiones de seguridad usted dejó de llevar mucho dinero en efectivo","En el último año, por cuestiones de seguridad usted dejó de usar joyas",
                "En esta calle o zona la policía cuida o vigila bien","En esta calle o zona la policía comete abusos","En esta calle o zona la policía acude a  los llamados","En esta calle o zona la policía pide mordidas","En esta calle o zona la policía hace rondines","En esta calle o zona la policía comete delitos",
                "Del 1 al 5, con el 5 como mejor calificación, califique la confianza que usted tiene en la policía","Del 1 al 5, con el 5 como mejor calificación, califique la confianza que usted tiene en el ministerio público para denunciar","Del 1 al 5, con el 5 como mejor calificación, califique la confianza que usted tiene en las escuelas públicas de la zona","Del 1 al 5, con el 5 como mejor calificación, califique la confianza que usted tiene en su presidente municipal","Del 1 al 5, con el 5 como mejor calificación, califique la confianza que usted tiene en el gobernador",
                "Del 1 al 5, califique el trabajo de la policía","Del 1 al 5, califique el trabajo del ministerio público para denunciar",#"Del 1 al 5, califique el trabajo de los empleados del gobierno",
                "Del 1 al 5, califique el trabajo de su presidente municipal","Del 1 al 5, califique el trabajo del gobernador",
                "Del 1 al 5, califique el trato que recibe de la policía","Del 1 al 5, califique el trato que recibe del ministerio público para denunciar","Del 1 al 5, califique el trato que recibe de los empleados del gobierno","Del 1 al 5, califique el trato que recibe de su presidente municipal","Del 1 al 5, califique el trato que recibe del gobernador",
                "Sexo","Número de personas que viven en esta casa","¿Cuántos son hombre?","¿Cuántos son mujeres?","Número de menores de 18 años","¿La madre vive en esta casa?","¿El padre vive en esta casa?","¿La casa es propia?","Tiempo que ha vivido en Q. Roo","Tiempo que ha vivido en esta casa"                   
                
                
      )[ii]
      output[[paste0("plotset",ii)]] <- renderPlot({
        if(is.numeric(datasetCompletes[[ii]])){
  ...
  output$txt4 <- renderUI({
    word <- paste("<b>",tags$i("o Descripción de las reglas de asociación y lectura del grafo:"),"</b>",
                  "</b>",("Las reglas de asociación se realizan con la técnica de minería de datos llamada apriori, la cual consiste en el razonamiento que parte de la causa (antecedente) al efecto (consecuente), tiene como objetivo encontrar relaciones dentro un conjunto de transacciones, en concreto, items o atributos que tienden a ocurrir de forma conjunta. En este contexto, el término transacción hace referencia a cada grupo de eventos que están asociados de alguna forma. 
                          ...
                          Una regla de asociación se define como una implicación del tipo “si X entonces Y” (X⇒Y), donde X e Y son itemsets o items individuales. El lado izquierdo de la regla recibe el nombre de antecedente y el lado derecho el nombre de consecuente. Por ejemplo, la regla {A,B} => {C} significa que, cuando ocurren A y B, también ocurre C.
                          Se extraen tanto itemsets o atributos frecuentes como reglas de asociación que superen un determinado soporte y confianza que el usuario especifique. Los argumentos de esta función son:")
  ...
  output$txt45 <- renderText({
    word <- paste("</b>",tags$ul(
      tags$li(strong("Soporte"), " - soporte mínimo que debe tener un itemset o artículo para ser considerado frecuente, es el número de transacciones que contienen X dividido entre el total de transacciones, es decir ¿Cuántas veces aparece el antecedente dentro de la transacción?. Por defecto es 0.25."),
      tags$li(strong("Confianza"), " - confianza mínima que debe de tener una regla para ser incluida en los resultados. La confianza de una regla implica “Si X entonces Y” y se interpreta como la probabilidad de que una transacción que contiene los items de X, también contenga los items de Y. Por defecto 0.25."),
      tags$li(strong("Mín. Combinación reglas")," - número mínimo de items que debe tener un itemset para ser incluido en los resultados. Por defecto 2."),
      tags$li(strong("Máx. Combinación reglas")," - número máximo de items que puede tener un itemset para ser incluido en los resultados. Por defecto 7."),
      tags$li(strong("Máximo de tiempo")," - tiempo máximo que puede estar el algoritmo buscando subconjuntos. Por defecto 3 segundos.")
    ))
    
    HTML(paste(word))
  })
  
  output$txt46 <- renderUI({
    word <- paste(("Además de la confianza y el soporte, existen otras métricas que permiten cuantificar la calidad de las reglas y la probabilidad de que reflejen relaciones reales, estas se visualizan sobre la ventana emergente al uno colocarse sobre algún círculo del grafo que nos muestre una regla de asociación:"),
                  tags$ul(tags$li(strong("Lift")," - el estadístico lift compara la frecuencia observada de una regla con la frecuencia esperada simplemente por azar (si la regla no existe realmente). Cuanto más se aleje el valor de lift de 1, más evidencias de que la regla no se debe a un artefacto aleatorio, es decir, mayor la evidencia de que la regla representa un patrón real."),
                          tags$li(strong("Coverage")," - es el soporte de la parte izquierda de la regla (antecedente). Se interpreta como la frecuencia con la que el antecedente aparece en el conjunto de transacciones."),
                          tags$li(strong("Count")," - es el valor exacto de veces que aparece la regla dentro de la transacción, es decir es el recuento de soporte mínimo absoluto que representa el porcentaje.")
                  ))
    HTML(paste(word))
  })
  
  output$txt44 <- renderUI({
    word <- paste (tags$i("Los nodos del grafo indican las reglas de asociación que se obtienen con las columnas seleccionadas, estas se conectan por medio de flechas que van del antecedente al consecuente. En el medio de estas flechas de conexiones, podemos encontrar un círculo cuyo tam. representa el valor de soporte o frecuencia de la regla mientras que el color representa la confianza. Podemos ver que la confianza se representa desde rosa pálido hasta el rojo, siendo este último el valor más alto."))
    HTML(paste(word))
  })
  
  output$txt47 <- renderUI({
    word <- paste (tags$i("Reglas de asociación y algoritmo Apriori con R by Joaquín Amat Rodrigo, available under a Attribution 4.0 International (CC BY 4.0) at https://www.cienciadedatos.net/documentos/43_reglas_de_asociacion"))
    HTML(paste(word))
  })
  ...
  output$rows1 <- renderUI({ if(input$selec == "Características de población y migración"){ Text <- "Info.Sexo"
  tex <- "Sexo"
  showvar <- input$show_vars
  selec <- input$selec
  ...
  output$rows2 <- renderUI({ if(input$selec == "Características de población y migración"){ Text <- "Info.Edad"
  tex <- "Edad"
  showvar <- input$show_vars
  selec <- input$selec
  ...
  output$rows3 <- renderUI({ if(input$selec == "Características de población y migración"){ Text <- "Direccion.Calle"
  tex <- "Calle"
  showvar <- input$show_vars
  selec <- input$selec
  ...
  output$rows4 <- renderUI({ if(input$selec == "Características de población y migración"){ Text <- "Direccion.Numero.Exterior"
  tex <- "N.Exterior"
  showvar <- input$show_vars
  selec <- input$selec
  ...
  output$rows5 <- renderUI({ if(input$selec == "Características de población y migración"){ Text <- "Direccion.Numero.Interior"
  tex <- "N.Interior"
  showvar <- input$show_vars
  selec <- input$selec
  ...
  output$rows6 <- renderUI({ if(input$selec == "Características de población y migración"){ Text <- "Direccion.Manza0"
  tex <- "Mz"
  showvar <- input$show_vars
  selec <- input$selec
  ...
  output$rows7 <- renderUI({ if(input$selec == "Características de población y migración"){ Text <- "Direccion.Super.Manza0"
  tex <- "Smz"
  showvar <- input$show_vars
  selec <- input$selec
  ...
  output$rows8 <- renderUI({ if(input$selec == "Características de población y migración"){ Text <- "Direccion.Colonia"
  tex <- "Colonia"
  showvar <- input$show_vars
  selec <- input$selec
  ...
  output$rows9 <- renderUI({ if(input$selec == "Características de población y migración"){ Text <- "Direccion.Codigo.Postal"
  tex <- "Codigo.P"
  showvar <- input$show_vars
  selec <- input$selec
  ...
  output$rows10 <- renderUI({ if(input$selec == "Características de población y migración"){ Text <- "V01_Casa_Adultos"
  tex <- "Núm.Adultos"
  showvar <- input$show_vars
  selec <- input$selec
  ...
  output$rows11 <- renderUI({ if(input$selec == "Características de población y migración"){ Text <- "V02_Casa_Ninos"
  tex <- "Núm.Niños"
  showvar <- input$show_vars
  selec <- input$selec
  ...
  output$rows12 <- renderUI({ if(input$selec == "Características de población y migración"){ Text <- "DF_kinder"
  tex <- "Cuántos.Kinder"
  showvar <- input$show_vars
  selec <- input$selec
  ...
  output$rows13 <- renderUI({ if(input$selec == "Características de población y migración"){ Text <- "DF_primaria"
  ...
  output$rows14 <- renderUI({ if(input$selec == "Características de población y migración"){ Text <- "DF_secundaria"
  ...
  output$rows15 <- renderUI({ if(input$selec == "Características de población y migración"){ Text <- "DF_bachillerato"
  ...
  output$rows16 <- renderUI({ if(input$selec == "Características de población y migración"){ Text <- "DF_licenciatura"
  ...
  output$rows17 <- renderUI({ if(input$selec == "Características de población y migración"){ Text <- "DF_tr_autobus"
  tex <- "Autobus"
  showvar <- input$show_vars
  selec <- input$selec
  ...
  output$rows18 <- renderUI({ if(input$selec == "Características de población y migración"){ Text <- "DF_tr_colectivo"
  tex <- "Colectivo"
  showvar <- input$show_vars
  selec <- input$selec
  ...
  output$rows19 <- renderUI({ if(input$selec == "Características de población y migración"){ Text <- "DF_tr_taxi"
  tex <- "Taxi"
  showvar <- input$show_vars
  selec <- input$selec
  ...
  output$rows20 <- renderUI({ if(input$selec == "Características de población y migración"){ Text <- "DF_tr_mototaxi"
  tex <- "Mototaxi"
  showvar <- input$show_vars
  selec <- input$selec
  ...
  output$rows21 <- renderUI({ if(input$selec == "Características de población y migración"){ Text <- "DF_tr_moto"
  tex <- "Moto"
  showvar <- input$show_vars
  selec <- input$selec
  ...
  output$rows22 <- renderUI({ if(input$selec == "Características de población y migración"){ Text <- "DF_tr_auto"
  tex <- "Auto"
  showvar <- input$show_vars
  selec <- input$selec
  ...
  output$rows23 <- renderUI({ if(input$selec == "Características de población y migración"){ Text <- "DF_v_col"
  tex <- "Mercado.Colonia"
  showvar <- input$show_vars
  selec <- input$selec
  ...
  output$rows24 <- renderUI({ if(input$selec == "Características de población y migración"){ Text <- "DF_v_abarrote"
  tex <- "Abarrote"
  showvar <- input$show_vars
  selec <- input$selec
  ...
  output$rows25 <- renderUI({ if(input$selec == "Características de población y migración"){ Text <- "DF_v_super"
  tex <- "Super"
  showvar <- input$show_vars
  selec <- input$selec
  ...
  output$rows26 <- renderUI({ if(input$selec == "Características de población y migración"){ Text <- "DF_v_conv"
  ...
  output$rows27 <- renderUI({ if(input$selec == "Características de población y migración"){ Text <- "DF_v_plaza"
  tex <- "Plaza"
  showvar <- input$show_vars
  selec <- input$selec
  ...
  output$rows28 <- renderUI({ if(input$selec == "Características de población y migración"){ Text <- "DF_urg_casa"
  ...
  output$rows29 <- renderUI({ if(input$selec == "Características de población y migración"){ Text <- "DF_urg_partcom"
  ...
  output$rows30 <- renderUI({ if(input$selec == "Características de población y migración"){ Text <- "DF_urg_partcan"
  ...
  output$rows31 <- renderUI({ if(input$selec == "Características de población y migración"){ Text <- "DF_urg_hosp"
  ...
  output$rows32 <- renderUI({ if(input$selec == "Características de población y migración"){ Text <- "DF_urg_cruz"
  ...
  output$rows33 <- renderUI({ if(input$selec == "Características de población y migración"){ Text <- "DF_urg_farmacia"
  ...
  output$rows34 <- renderUI({ if(input$selec == "Características de población y migración"){ Text <- "DF_urg_otro"
  tex <- "Emergencia.Otro"
  showvar <- input$show_vars
  selec <- input$selec
  ...
  output$rows35 <- renderUI({ if(input$selec == "Características de población y migración"){ Text <- "DF_a_parque"
  ...
  output$rows36 <- renderUI({ if(input$selec == "Características de población y migración"){ Text <- "DF_a_unidad"
  ...
  output$rows37 <- renderUI({ if(input$selec == "Características de población y migración"){ Text <- "DF_a_jardines"
  ...
  output$rows38 <- renderUI({ if(input$selec == "Características de población y migración"){ Text <- "DF_a_casa"
  ...
  output$rows39 <- renderUI({ if(input$selec == "Características de población y migración"){ Text <- "DF_a_biblioteca"
  ...
  output$rows40 <- renderUI({ if(input$selec == "Características de población y migración"){ Text <- "DF_a_otro"
  ...
  output$rows41 <- renderUI({ if(input$selec == "Características de población y migración"){ Text <- "DE01_hogar_trabajan"
  ...
  output$rows42 <- renderUI({ if(input$selec == "Características de población y migración"){ Text <- "DE02_trabajo"
  tex <- "Jefe.Trabajo"
  showvar <- input$show_vars
  selec <- input$selec
  ...
  output$rows43 <- renderUI({ if(input$selec == "Características de población y migración"){ Text <- "DE03_puesto"
  tex <- "Jefe.Puesto"
  showvar <- input$show_vars
  selec <- input$selec
  ...
  output$rows44 <- renderUI({ if(input$selec == "Características de población y migración"){ Text <- "DE05_ingreso_sem"
  tex <- "Ingreso.Sem"
  showvar <- input$show_vars
  selec <- input$selec
  ...
  output$rows45 <- renderUI({ if(input$selec == "Características de población y migración"){ Text <- "DE06_esc_jefe"
  ...
  output$rows46 <- renderUI({ if(input$selec == "Características de población y migración"){ Text <- "DE07_sIngreso"
  ...
  output$rows47 <- renderUI({ if(input$selec == "Características de población y migración"){ Text <- "DE08_tmp"
  ...
  output$rows48 <- renderUI({ if(input$selec == "Características de población y migración"){ Text <- "DE09_col"
  tex <- "Colonia.Trabajo"
  showvar <- input$show_vars
  selec <- input$selec
  ...
  output$rows49 <- renderUI({ if(input$selec == "Características de población y migración"){ Text <- "DE10_mun"
  ...
  output$rows50 <- renderUI({ if(input$selec == "Características de población y migración"){ Text <- "IyC02_Estado_Origen"
  tex <- "Estado.Origen"
  showvar <- input$show_vars
  selec <- input$selec
  ...
  output$rows51 <- renderUI({ if(input$selec == "Características de población y migración"){ Text <- "IyC03_Tiempo"
  tex <- "Tiempo.En.Isla"
  showvar <- input$show_vars
  selec <- input$selec
  ...
  output$rows52 <- renderUI({ if(input$selec == "Características de población y migración"){ Text <- "IyC04_Motivo_Localidad_Parientes"
  ...
  output$rows53 <- renderUI({ if(input$selec == "Características de población y migración"){ Text <- "IyC04_Motivo_Localidad_Amigos"
  ...
  output$rows54 <- renderUI({ if(input$selec == "Características de población y migración"){ Text <- "IyC04_Motivo_Localidad_Trabajo"
  ...
  output$rows55 <- renderUI({ if(input$selec == "Características de población y migración"){ Text <- "IyC04_Motivo_Localidad_Negocio"
  ...
  output$rows56 <- renderUI({ if(input$selec == "Características de población y migración"){ Text <- "IyC04_Motivo_Localidad_Oportunidad"
  ...
  output$rows57 <- renderUI({ if(input$selec == "Características de población y migración"){ Text <- "IyC04_Motivo_Localidad_Otro"
  ...
  output$rows58 <- renderUI({ if(input$selec == "Características de población y migración"){ Text <- "IyC05_Acudo_Vecinos"
  tex <- "Acudo.Vecinos"
  showvar <- input$show_vars
  selec <- input$selec
  ...
  output$rows59 <- renderUI({ if(input$selec == "Características de población y migración"){ Text <- "IyC05_Acudo_Familia"
  tex <- "Acudo.Familia"
  showvar <- input$show_vars
  selec <- input$selec
  ...
  output$rows60 <- renderUI({ if(input$selec == "Características de población y migración"){ Text <- "IyC05_Acudo_Autoridad"
  tex <- "Acudo.Autoridad"
  showvar <- input$show_vars
  selec <- input$selec
  ...
  output$rows61 <- renderUI({ if(input$selec == "Características de población y migración"){ Text <- "IyC05_Acudo_Iglesia"
  tex <- "Acudo.Iglesia"
  showvar <- input$show_vars
  selec <- input$selec
  ...
  output$rows62 <- renderUI({ if(input$selec == "Características de población y migración"){ Text <- "IyC05_Acudo_Otro"
  tex <- "Acudo.Otro"
  showvar <- input$show_vars
  selec <- input$selec
  ...
  output$rows65 <- renderUI({ if(input$selec == "Características de población y migración"){ Text <- "IyC08_Ventajas_Casa"
  tex <- "Ventajas.Casa"
  showvar <- input$show_vars
  selec <- input$selec
  ...
  output$rows66 <- renderUI({ if(input$selec == "Características de población y migración"){ Text <- "IyC08_Ventajas_Trabajo"
  ...
  output$rows67 <- renderUI({ if(input$selec == "Características de población y migración"){ Text <- "IyC08_Ventajas_Familia"
  ...
  output$rows68 <- renderUI({ if(input$selec == "Características de población y migración"){ Text <- "IyC08_Ventajas_Tiempo"
  tex <- "Ventajas.Tiempo"
  showvar <- input$show_vars
  selec <- input$selec
  ...
  output$rows69 <- renderUI({ if(input$selec == "Características de población y migración"){ Text <- "IyC08_Ventajas_Tranquilo"
  ...
  output$rows70 <- renderUI({ if(input$selec == "Características de población y migración"){ Text <- "IyC08_Ventajas_Seguro"
  tex <- "Ventajas.Seguro"
  showvar <- input$show_vars
  selec <- input$selec
  ...
  output$rows71 <- renderUI({ if(input$selec == "Características de población y migración"){ Text <- "IyC08_Ventajas_Otro"
  tex <- "Ventajas.Otro"
  showvar <- input$show_vars
  selec <- input$selec
  ...
  output$rows72 <- renderUI({ if(input$selec == "Características de población y migración"){ Text <- "IyC09_Emigrar"
  tex <- "Emigrar"
  showvar <- input$show_vars
  selec <- input$selec
  ...
  output$rows73 <- renderUI({ if(input$selec == "Características de población y migración"){ Text <- "IyC10_Pertenencia"
  tex <- "Pertenencia"
  showvar <- input$show_vars
  selec <- input$selec
  ...
  output$rows74 <- renderUI({ if(input$selec == "Características de población y migración"){ Text <- "IyC11_Frecuencia_Cabecera_Isla"
  ...
  output$rows75 <- renderUI({ if(input$selec == "Características de población y migración"){ Text <- "IyC12_Motivo_Viaja_Asuntos_Admin"
  ...
  output$rows76 <- renderUI({ if(input$selec == "Características de población y migración"){ Text <- "IyC12_Motivo_Viaja_Pago_FALTA_DE_SERVICIOS"
  ...
  output$rows77 <- renderUI({  if(input$selec == "Características de población y migración"){ Text <- "IyC12_Motivo_Viaja_Trabajo"
  ...
  output$rows78 <- renderUI({  if(input$selec == "Características de población y migración"){ Text <- "IyC12_Motivio_Viaja_Recreacion"
  ...
  output$rows79 <- renderUI({ if(input$selec == "Características de población y migración"){ Text <- "IyC12_Motivo_Viaja_Familia"
  ...
  output$rows80 <- renderUI({  if(input$selec == "Características de población y migración"){ Text <- "IyC12_Motivo_Viaja_Otro"
  ...
  output$rows81 <- renderUI({ if(input$selec == "Características de población y migración"){ Text <- "VI01_La_vivienda_es"
  tex <- "La.Vivienda.Es"
  showvar <- input$show_vars
  selec <- input$selec
  ...
  output$rows82 <- renderUI({ if(input$selec == "Características de población y migración"){ Text <- "VI04_Contado"
  ...
  output$rows83 <- renderUI({ if(input$selec == "Características de población y migración"){ Text <- "VI04_Herencia"
  ...
  output$rows84 <- renderUI({ if(input$selec == "Características de población y migración"){ Text <- "VI01_Prestada"
  ...
  output$rows85 <- renderUI({ if(input$selec == "Características de población y migración"){ Text <- "VI04_Mensual"
  ...
  output$rows86 <- renderUI({ if(input$selec == "Características de población y migración"){ Text <- "VI05_CPropiaMensual"
  ...
  output$rows87 <- renderUI({ if(input$selec == "Características de población y migración"){ Text <- "VI06_CPropiaAdquirida"
  ...
  output$rows89 <- renderUI({  if(input$selec == "Características de población y migración"){ Text <- "VI08NINUNDACIONES"
  ...
  output$rows90 <- renderUI({ if(input$selec == "Características de población y migración"){ Text <- "VI09PeHuracan"
  tex <- "Pasado.Huracan"
  showvar <- input$show_vars
  selec <- input$selec
  ...
  output$rows91 <- renderUI({ if(input$selec == "Características de población y migración"){ Text <- "VI14SabeZo0Riesgo"
  tex <- "Sabe.Zo.Riesgo"
  showvar <- input$show_vars
  selec <- input$selec
  ...
  output$rows92 <- renderUI({ if(input$selec == "Características de población y migración"){ Text <- "VI15SabeAfectaciones"
  ...
  output$rows93 <- renderUI({  if(input$selec == "Características de población y migración"){ Text <- "AE1_AreasVerdes"
  tex <- "Obs.AreasVerdes"
  showvar <- input$show_vars
  selec <- input$selec
  ...
  output$rows94 <- renderUI({  if(input$selec == "Características de población y migración"){ Text <- "AE3_Banquetas"
  tex <- "Obs.Banquetas"
  showvar <- input$show_vars
  selec <- input$selec
  ...
  output$rows95 <- renderUI({  if(input$selec == "Características de población y migración"){ Text <- "AE4_Lumi0rias"
  tex <- "Obs.Luminarias"
  showvar <- input$show_vars
  selec <- input$selec
  ...
  output$rows96 <- renderUI({ if(input$selec == "Características de población y migración"){ Text <- "AE5_Transporte"
  tex <- "Obs.Transporte"
  showvar <- input$show_vars
  selec <- input$selec
  ...
  output$rows97 <- renderUI({  if(input$selec == "Características de población y migración"){ Text <- "AE6_Patrullas"
  tex <- "Obs.Patrullas"
  showvar <- input$show_vars
  selec <- input$selec
  ...
  output$rows98 <- renderUI({ if(input$selec == "Características de población y migración"){ Text <- "AE7_Lotes"
  tex <- "Obs.Lotes"
  showvar <- input$show_vars
  selec <- input$selec
  ...
  output$rows99 <- renderUI({ if(input$selec == "Características de población y migración"){ Text <- "AE9_PeSeguridad"
  tex <- "Obs.PeSeguridad"
  showvar <- input$show_vars
  selec <- input$selec
  ...
  output$rows100 <- renderUI({ if(input$selec == "Características de población y migración"){ Text <- "AE10_PeComodidad"
  tex <- "Obs.PeComodidad"
  showvar <- input$show_vars
  selec <- input$selec
  ...
  output$rows101 <- renderUI({  if(input$selec == "Características de población y migración"){ Text <- "AE11_PeRiesgo"
  tex <- "Obs.PeRiesgo"
  showvar <- input$show_vars
  selec <- input$selec
  ...
  output$rows102 <- renderUI({ if(input$selec != "Características de población y migración"){ Text <- "En.Casa.Alguien.Tiene.Discapacidad"
  showvar <- input$show_vards
  selec <- input$selec
  secc2(Text, showvar, selec) } })
  
  output$rows103 <- renderUI({ if(input$selec != "Características de población y migración"){ Text <- "En.Casa.Alguien.No.Habla.Espanol"
  text <- "En.Casa.Alguien.No.Habla.Español"
  showvar <- input$show_vards
  selec <- input$selec
  secc3(Text, showvar, selec, text) } })
  
  output$rows104 <- renderUI({ if(input$selec != "Características de población y migración"){ Text <- "En.Casa.Alguien.Necesita.Ayuda.Obesidad"
  showvar <- input$show_vards
  selec <- input$selec
  secc2(Text, showvar, selec) } })
  
  output$rows105 <- renderUI({ if(input$selec != "Características de población y migración"){ Text <- "En.Casa.Alguien.Necesita.Ayuda.Fumar"
  showvar <- input$show_vards
  selec <- input$selec
  secc2(Text, showvar, selec) } })
  
  output$rows106 <- renderUI({ if(input$selec != "Características de población y migración"){ Text <- "En.Casa.Alguien.Necesita.Ayuda.Beber"
  showvar <- input$show_vards
  selec <- input$selec
  secc2(Text, showvar, selec) } })
  
  output$rows107 <- renderUI({ if(input$selec != "Características de población y migración"){ Text <- "Ultimo.Ano.Por.Seguridad.Ha.Pensado.Cambiarse.Casa"
  text <- "Último.Año.Por.Seguridad.Ha.Pensado.Cambiarse.Casa"
  showvar <- input$show_vards
  selec <- input$selec
  secc3(Text, showvar, selec, text) } })
  
  output$rows108 <- renderUI({ if(input$selec != "Características de población y migración"){ Text <- "Ultimo.Ano.Por.Seguridad.Ha.Pensado.Cambiarse.Ciudad"
  text <- "Último.Año.Por.Seguridad.Ha.Pensado.Cambiarse.Ciudad"
  showvar <- input$show_vards
  selec <- input$selec
  secc3(Text, showvar, selec, text) } })
  
  output$rows109 <- renderUI({ if(input$selec != "Características de población y migración"){ Text <- "Ultimo.Ano.Por.Seguridad.Ha.Pensado.Cambiarse.Estado"
  text <- "Último.Año.Por.Seguridad.Ha.Pensado.Cambiarse.Estado"
  showvar <- input$show_vards
  selec <- input$selec
  secc3(Text, showvar, selec, text) } })
  
  output$rows110 <- renderUI({ if(input$selec != "Características de población y migración"){ Text <- "Ultimo.Ano.Por.Seguridad.Ha.Pensado.Dejo.Salir.Noche"
  text <- "Último.Año.Por.Seguridad.Ha.Pensado.Dejó.Salir.Noche"
  showvar <- input$show_vards
  selec <- input$selec
  secc3(Text, showvar, selec, text) } })
  
  output$rows111 <- renderUI({ if(input$selec != "Características de población y migración"){ Text <- "Ultimo.Ano.Por.Seguridad.Dejo.De.Salir.Caminar"
  text <- "Último.Año.Por.Seguridad.Dejó.De.Salir.Caminar"
  showvar <- input$show_vards
  selec <- input$selec
  secc3(Text, showvar, selec, text) } })
  
  output$rows112 <- renderUI({ if(input$selec != "Características de población y migración"){ Text <- "Ultimo.Ano.Por.Seguridad.Impidio.Ninos.Salieran.Calle"
  text <- "Último.Año.Por.Seguridad.Impidió.Niños.Salieran.Calle"
  showvar <- input$show_vards
  selec <- input$selec
  secc3(Text, showvar, selec, text) } })
  
  output$rows113 <- renderUI({ if(input$selec != "Características de población y migración"){ Text <- "Ult.Ano.Por.Seguridad.Evito.Relacionarse.Nuevas.Personas"
  text <- "Último.Año.Por.Seguridad.Evitó.Relacionarse.Nuevas.Personas"
  showvar <- input$show_vards
  selec <- input$selec
  secc3(Text, showvar, selec, text) } })
  
  output$rows114 <- renderUI({ if(input$selec != "Características de población y migración"){Text <- "Ultimo.Ano.Por.Seguridad.Dejo.De.Visitar.Parientes.Amigos"
  text <- "Último.Año.Por.Seguridad.Dejó.De.Visitar.Parientes.Amigos"
  showvar <- input$show_vards
  selec <- input$selec
  secc3(Text, showvar, selec, text) } })
  
  output$rows115 <- renderUI({ if(input$selec != "Características de población y migración"){Text <- "Ultimo.Ano.Por.Seguridad.Dejo.Usar.Taxi"
  text <- "Último.Año.Por.Seguridad.Dejó.Usar.Taxi"
  showvar <- input$show_vards
  selec <- input$selec
  secc3(Text, showvar, selec, text) } })
  
  output$rows116 <- renderUI({ if(input$selec != "Características de población y migración"){Text <- "Ultimo.Ano.Por.Seguridad.Dejo.De.Llevar.Mucho.Efectivo"
  text <- "Último.Año.Por.Seguridad.Dejó.De.Llevar.Mucho.Efectivo"
  showvar <- input$show_vards
  selec <- input$selec
  secc3(Text, showvar, selec, text) } })
  
  output$rows117 <- renderUI({ if(input$selec != "Características de población y migración"){Text <- "Ultimo.Ano.Por.Seguridad.Dejo.De.Usar.Joyas"
  text <- "Último.Año.Por.Seguridad.Dejó.De.Usar.Joyas"
  showvar <- input$show_vards
  selec <- input$selec
  secc3(Text, showvar, selec, text) } })
  
  output$rows118 <- renderUI({ if(input$selec != "Características de población y migración"){Text <- "Policia.Cuida.Vigila.Bien"
  text <- "Policía.Cuida.Vigila.Bien"
  showvar <- input$show_vards
  selec <- input$selec
  secc3(Text, showvar, selec, text) } })
  
  output$rows119 <- renderUI({ if(input$selec != "Características de población y migración"){Text <- "Policia.Comete.Abusos"
  text <- "Policía.Comete.Abusos"
  showvar <- input$show_vards
  selec <- input$selec
  secc3(Text, showvar, selec, text) } })
  
  output$rows120 <- renderUI({ if(input$selec != "Características de población y migración"){Text <- "Policia.Acude.Llamados"
  text <- "Policía.Acude.Llamados"
  showvar <- input$show_vards
  selec <- input$selec
  secc3(Text, showvar, selec, text) } })
  
  output$rows121 <- renderUI({ if(input$selec != "Características de población y migración"){Text <- "Policia.Pide.Mordidas"
  text <- "Policía.Pide.Mordidas"
  showvar <- input$show_vards
  selec <- input$selec
  secc3(Text, showvar, selec, text) } })
  
  output$rows122 <- renderUI({ if(input$selec != "Características de población y migración"){Text <- "Policia.Hace.Rondines"
  text <- "Policía.Hace.Rondines"
  showvar <- input$show_vards
  selec <- input$selec
  secc3(Text, showvar, selec, text) } })
  
  output$rows123 <- renderUI({ if(input$selec != "Características de población y migración"){Text <- "Policia.Comete.Delitos"
  text <- "Policía.Comete.Delitos"
  showvar <- input$show_vards
  selec <- input$selec
  secc3(Text, showvar, selec, text) } })
  
  output$rows124 <- renderUI({ if(input$selec != "Características de población y migración"){Text <- "Confianza.Que.Tiene.En.La.Policia"
  text <- "Confianza.Que.Tiene.En.La.Policía"
  showvar <- input$show_vards
  selec <- input$selec
  secc3(Text, showvar, selec,text) } })
  
  output$rows125 <- renderUI({ if(input$selec != "Características de población y migración"){Text <- "Confianza.Que.Tiene.En.Ministerio.Publico"
  text <- "Confianza.Que.Tiene.En.Ministerio.Público"
  showvar <- input$show_vards
  selec <- input$selec
  secc3(Text, showvar, selec,text) } })
  
  output$rows126 <- renderUI({ if(input$selec != "Características de población y migración"){Text <- "Confianza.Que.Tiene.En.La.Institucion.Educativa.De.Zona"
  text <- "Confianza.Que.Tiene.En.La.Institución.Educativa.De.Zona"
  showvar <- input$show_vards
  selec <- input$selec
  secc3(Text, showvar, selec, text) } })
  
  output$rows127 <- renderUI({ if(input$selec != "Características de población y migración"){Text <- "Confianza.Que.Tiene.En.Su.Presidente.Municipal"
  showvar <- input$show_vards
  selec <- input$selec
  secc2(Text, showvar, selec) } })
  
  output$rows128 <- renderUI({ if(input$selec != "Características de población y migración"){Text <- "Confianza.Que.Tiene.En.El.Gobernador"
  showvar <- input$show_vards
  selec <- input$selec
  secc2(Text, showvar, selec) } })
  
  output$rows129 <- renderUI({ if(input$selec != "Características de población y migración"){Text <- "Calificacion.Trabajo.De.La.Policia"
  text <- "Calificación.Trabajo.De.La.Policia"
  showvar <- input$show_vards
  selec <- input$selec
  secc3(Text, showvar, selec, text) } })
  
  output$rows130 <- renderUI({ if(input$selec != "Características de población y migración"){Text <- "Calificacion.Trabajo.Del.Ministerio.Publico.Para.Denunciar"
  text <- "Calificación.Trabajo.Del.Ministerio.Público.Para.Denunciar"
  showvar <- input$show_vards
  selec <- input$selec
  secc3(Text, showvar, selec, text) } })
  
  output$rows131 <- renderUI({ if(input$selec != "Características de población y migración"){Text <- "Calificacion.Trabajo.De.Empleados.De.Gobierno"
  ...
  output$rows132 <- renderUI({ if(input$selec != "Características de población y migración"){Text <- "Calificacion.Trabajo.Presidente.Municipal"
  text <- "Calificación.Trabajo.Presidente.Municipal"
  showvar <- input$show_vards
  selec <- input$selec
  secc3(Text, showvar, selec, text) } })
  
  output$rows133 <- renderUI({ if(input$selec != "Características de población y migración"){Text <- "Calificacion.Trabajo.Del.Gobernador"
  text <- "Calificación.Trabajo.Del.Gobernador"
  showvar <- input$show_vards
  selec <- input$selec
  secc3(Text, showvar, selec, text) } })
  
  output$rows134 <- renderUI({ if(input$selec != "Características de población y migración"){Text <- "Calificacion.Trato.Que.Da.La.Policia"
  text <- "Calificación.Trato.Que.Da.La.Policia"
  showvar <- input$show_vards
  selec <- input$selec
  secc3(Text, showvar, selec, text) } })
  
  output$rows135 <- renderUI({ if(input$selec != "Características de población y migración"){Text <- "Calificacion.Trato.Que.Da.Ministerio.Publico.Para.Denunciar"
  text <- "Calificación.Trato.Que.Da.Ministerio.Público.Para.Denunciar"
  showvar <- input$show_vards
  selec <- input$selec
  secc3(Text, showvar, selec, text) } })
  
  output$rows136 <- renderUI({ if(input$selec != "Características de población y migración"){Text <- "Calificacion.Trato.Recibe.De.Los.Empleados.De.Gobierno"
  text <- "Calificación.Trato.Recibe.De.Los.Empleados.De.Gobierno"
  showvar <- input$show_vards
  selec <- input$selec
  secc3(Text, showvar, selec, text) } })
  
  output$rows137 <- renderUI({ if(input$selec != "Características de población y migración"){Text <- "Calificacion.Trato.Recibe.De.Su.Presidente.Municipal"
  text <- "Calificación.Trato.Recibe.De.Su.Presidente.Municipal"
  showvar <- input$show_vards
  selec <- input$selec
  secc3(Text, showvar, selec, text) } })
  
  output$rows138 <- renderUI({ if(input$selec != "Características de población y migración"){Text <- "Calificacion.Trato.Recibe.Del.Gobernador"
  text <- "Calificación.Trato.Recibe.Del.Gobernador"  
  showvar <- input$show_vards
  selec <- input$selec
  secc3(Text, showvar, selec, text) } })
  
  output$rows139 <- renderUI({ if(input$selec != "Características de población y migración"){Text <- "Num.Personas.Viven.Esta.Casa"
  text <- "Núm.Personas.Viven.Esta.Casa"
  showvar <- input$show_vards
  selec <- input$selec
  secc3(Text, showvar, selec, text) } })
  
  output$rows140 <- renderUI({ if(input$selec != "Características de población y migración"){Text <- "Cuantos.Hombres"
  text <- "Cuantos.Hombres"
  showvar <- input$show_vards
  selec <- input$selec
  secc3(Text, showvar, selec, text) } })
  
  output$rows141 <- renderUI({ if(input$selec != "Características de población y migración"){Text <- "Cuantas.Mujeres"
  text <- "Cuantas.Mujeres"
  showvar <- input$show_vards
  selec <- input$selec
  secc3(Text, showvar, selec, text) } })
  
  output$rows142 <- renderUI({ if(input$selec != "Características de población y migración"){Text <- "Cuantos.Menores"
  text <- "Cuántos.Menores"
  showvar <- input$show_vards
  selec <- input$selec
  secc3(Text, showvar, selec, text) } })
  
  output$rows143 <- renderUI({ if(input$selec != "Características de población y migración"){Text <- "La.Madre.De.Los.Menores.Vive.En.Casa"
  showvar <- input$show_vards
  selec <- input$selec
  secc2(Text, showvar, selec) } })
  
  output$rows144 <- renderUI({ if(input$selec != "Características de población y migración"){Text <- "El.Padre.De.Los.Menores.Vive.En.Casa"
  showvar <- input$show_vards
  selec <- input$selec
  secc2(Text, showvar, selec) } })
  
  output$rows145 <- renderUI({ if(input$selec != "Características de población y migración"){Text <- "Esta.Casa.Es.Propia"
  showvar <- input$show_vards
  selec <- input$selec
  secc2(Text, showvar, selec) } })
  
  
  output$rows147 <- renderUI({ if(input$selec != "Características de población y migración"){Text <- "Tiempo.Viviendo.En.Q.Roo"
  showvar <- input$show_vards
  selec <- input$selec
  secc2(Text, showvar, selec) } })
  
  output$rows148 <- renderUI({ if(input$selec != "Características de población y migración"){Text <- "Tiempo.Viviendo.En.Esta.Casa"
  showvar <- input$show_vards
  selec <- input$selec
  secc2(Text, showvar, selec) } })
